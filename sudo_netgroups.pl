...
foreach $pline (@passwd_tmp) {
  if ($pline !~ /^$/) {
        ($partpas1) = $pline =~ /([^:]*)/;                                      # Part one - example: +rootadm  ALL=(ALL) NOPASSWD
        ($partpas2) = $pline =~ /:(.+)$/;                                       # Part two - example: CMD_ORA,NOPASSWD:RM,NPASSWD:TRACE,NOPASSWD:CRFILE
         ...
         if ($partpas2 !~ /PASSWD/) {
                ...
                foreach $npwd (split /,/, $partpas2)  {
                        if ($npwd !~ /PASSWD/) {                                # Statement for first parameter without stentence "PASSWD" ex: CMD_ORA
                                $tpline = $npwd;
                                $tpline =~ s/^\s+//;
                                $fpwline = ("$partpas1" . ":" . "$tpline\n");   # Needed output - example +rootadm  ALL=(ALL) NOPASSWD:CMD_ORA
                                $all_passwd = ("$all_passwd" . "$fpwline\n");
                        } else {
                                $tpline = $npwd;
                                $tpline =~ s/^\s+//;                            # Delete spaces at the beginning
                                #
                                if ($tpline =~ /\(/ && $tpline =~ /\)/) {       # If string contain () - example: "(oormds) NOPASSWD: SU"
...
for $lastpr (@last) {
        ...
        # FINAL PRINT:
        $fout = ($fout . "$real_user" . ";" . "$real_alias" . ";" . "$as_user" . ";" . "$cmd_alias" . ";" . "$command" . ";" . "$pwflag" . ";" . "$excluded\n" );
        }
}
#print $fout;
#print "------------\n";
...
@a_fino = split /^/m ,$fout;
foreach $line (@a_fino) {
        ($partusr1) = $line =~ /([^;]*)/;                                      # Part one - example: +rootadm  ALL=(ALL) NOPASSW
        ($partusr2) = $line =~ /;(.+)$/;
        if ($partusr1 =~ /^\+/) {
                $netg = $partusr1;
                ($netg) =~ s/\+//g;
                ($netg) =~ s/\+//g;
                # IF array already exsist (LDAP query has been made) print all users belongs to this NetGroup
                if (defined $netu{$netg} && length $netu{$netg} > 0) {
                        @netusers = split /^/m ,($netu{$netg}) ;
                        foreach $line (@netusers) {
                                ($p2) = $line =~ /^(.+)$/;      #remove new line character(match only string)
                                print "$p2;$netg;$partusr2\n";
                        }
                }
                else {
                        # List of NETGROUP users - TTT(PROD)
                        $netu{$netg} = `ldapsearch -LLL -H ldaps://ldap.xxx.xxx:636 -D 'cn=hostname-x-rhel7+iphostnumber=xx.xx.xxx.xxx, \
                        ou=hosts,dc=ttt,dc=xxxxxx,dc=xx' -w 'PASSWD' -b "ou=netgroup,dc=ttt,dc=xxxxxx, \
                        dc=eu" "cn=$netg" | grep nisNetgroup | grep -v 'dummy' | awk -F"," '{print \$2}' | grep -v '^\$'`;
                        #
                        # If array exist and is > 0 (if LDAP query find NetGroup and users belongs to it):
                        if (defined $netu{$netg} && length $netu{$netg} > 0) {
                                # For aech user in NETGROUP print user + rest of line :)
                                @netusers = split /^/m ,($netu{$netg}) ;
                                foreach $line (@netusers) {
                                        ($p2) = $line =~ /^(.+)$/;      #remove new line character(match only string)
                                        print "$p2;$netg;$partusr2\n";
                                ...
                                # List of NETGROUP users - TTT2(PROD)
                                $netu{$netg} = `ldapsearch -LLL -H ldaps://ldap.xxx.xxx:636 -D 'cn=hostname-x-rhel7+iphostnumber=xx.xx.xxx.xxx, \
                                ou=hosts,dc=ttt,dc=xxxxxx,dc=xx' -w 'PASSWD' -b "ou=netgroup,dc=ttt2,dc=xxxxxx, \
                                dc=eu" "cn=$netg" | grep nisNetgroup | grep -v 'dummy' | awk -F"," '{print \$2}' | grep -v '^\$'`;
                                if (defined $netu{$netg} && length $netu{$netg} > 0) {
                                        @netusers = split /^/m ,($netu{$netg}) ;
                                        foreach $line (@netusers) {
                ...
                print "$partusr1;$partusr1;$partusr2\n";
        }
}
